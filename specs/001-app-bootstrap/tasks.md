...
- [ ] T063 [P] [US5] Implement error wrapping in `App.Bootstrap()` for each initialization phase (config, logger, platform) with clear error messages
- [ ] T064 [P] [US5] Implement dotnet CLI validation in `internal/platform/detect.go` with `ValidateDotnetCLI() error` function (check PATH, run `dotnet --version`)
- [ ] T065 [US5] Add dotnet validation to `App.Bootstrap()` after platform initialization (make it async and non-blocking per research.md Layer 2 optimization)
...
- [ ] T067 [US5] Add YAML parse error handling in `config.Load()` with line/column information from parser
- [ ] T068 [US5] Add config directory permission checking in `App.Bootstrap()` (warn if cannot create, use temp directory fallback)
- [ ] T069 [US5] Update error logging in panic recovery layers to write to stderr and log file (if logger initialized)
- [ ] T070 [US5] Add exit code constants in `cmd/lazynuget/main.go` (ExitSuccess=0, ExitUserError=1, ExitSystemError=2) and use consistently

### Integration Tests for User Story 5

- [ ] T071 [P] [US5] Create `tests/integration/errors_test.go` with TestInvalidYAMLError that provides invalid YAML and validates error message clarity
- [ ] T072 [P] [US5] Create `tests/integration/errors_test.go` with TestMissingDotnetCLI that removes dotnet from PATH and validates error message with installation instructions
- [ ] T073 [P] [US5] Create `tests/integration/errors_test.go` with TestConflictingSettings that provides conflicting config and validates resolution
- [ ] T074 [P] [US5] Create `tests/integration/errors_test.go` with TestPartialInitializationFailure that mocks component failure and validates cleanup
